<template>
  <div class="card shadow-sm">
    ...
    <div class="table-container">
        <div class="table-responsive">
            <table class="table table-hover m-0">
                ...
                <tbody>
                    <tr v-for="topUp in topUps" :key="topUp.id" class="align-middle">
    ...
    <div v-if="paginationData && paginationData.last_page > 1" class="card-footer bg-white">
        <nav aria-label="Page navigation">
            <ul class="pagination pagination-sm justify-content-center flex-wrap mb-0">
                <li class="page-item" :class="{ disabled: paginationData.current_page === 1 }">
                    <button class="page-link" @click.prevent="emit('go-to-page', paginationData.current_page - 1)" :disabled="paginationData.current_page === 1">
                ...
                <li class="page-item" :class="{ disabled: paginationData.current_page === paginationData.last_page }">
                    <button class="page-link" @click.prevent="emit('go-to-page', paginationData.current_page + 1)" :disabled="paginationData.current_page === paginationData.last_page">
...
.status-badge-warning { color: #856404; background-color: #fff3cd; border: 1px solid #ffeeba; }
.status-badge-success { color: #155724; background-color: #d4edda; border: 1px solid #c3e6cb; }
.status-badge-danger { color: #721c24; background-color: #f8d7da; border: 1px solid #f5c6cb; }
.status-badge-secondary { color: #383d41; background-color: #e2e3e5; border: 1px solid #d6d8db; }
.proof-container { width: 60px; height: 40px; overflow: hidden; border-radius: 4px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); }
.proof-thumb { width: 100%; height: 100%; object-fit: cover; cursor: pointer; transition: transform 0.2s ease-in-out; }
.proof-thumb:hover { transform: scale(1.08); }
.empty-state { padding: 1rem; text-align: center; }
.empty-state i { font-size: 2.5rem; display: block; }
.empty-state p { margin-bottom: 0; color: #6c757d; }
.pagination { margin-top: 0; }
.page-link { user-select: none; }
