...
<template>
  <div class="payment-confirmation px-2 px-sm-3">
    <h3 class="fw-bold mb-3 mb-md-4 text-center fs-4">Pembayaran {{ selectedPaymentMethodDetails.name }}</h3>
    <div v-if="selectedPaymentMethodDetails.id === 'BCA'" class="mb-3 mb-md-4">
      <p class="fw-bold mb-2">Intruksi Pembayaran</p>
      <ol class="mb-3 mb-md-4 ps-3 ps-md-4">
      ...
      <div class="text-center bg-light p-2 p-sm-3 rounded">
        <p class="mb-1 text-muted small">Nomir Rekening {{ selectedPaymentMethodDetails.name }}</p>
        <div class="d-flex flex-wrap justify-content-center align-items-center gap-2">
          <span class="fs-5 fs-md-4 fw-bold">{{ bcaVirtualAccount }}</span>
    ...
    <div class="border rounded p-2 p-sm-3 mb-3 mb-md-4">
      <h5 class="fw-bold mb-3 fs-6 fs-md-5">Ringkasan Pembayaran</h5>
      <div class="d-flex flex-column flex-sm-row justify-content-between mb-2 gap-1">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-stretch gap-2 mt-4 mt-md-5">
      <button class="btn btn-outline-secondary flex-fill flex-sm-grow-0" @click="$emit('back')" :disabled="form.processing">
        <i class="bi bi-arrow-left me-1"></i> Kembali 
      </button>
      <button class="btn btn-success flex-fill flex-sm-grow-0" @click="$emit('confirm-payment')" :disabled="form.processing || !form.proof">
        <span v-if="form.processing" class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
        <span class="btn-text">Sudah melakukan pembayaran</span>
      </button>
    </div>
  </div>
</template>
